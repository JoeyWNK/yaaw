  ...
  return {
    init: function(path, onready) {
      ...
      if (jsonrpc_interface.indexOf("http") == 0) {
        ...
        jsonrpc_ws.onerror = function(event) {
          ...
          main_alert("alert-error", "网络套接字错误，你需要刷新当前页面来重新开始");
          ws_callback = {};
        };
    ...
    tell_active: function(keys) {
      ...
      ARIA2.request("tellActive", keys,
        function(result) {
          ...
          if (select_lock) return;
          if (!result.result) {
            main_alert("alert-error", "<strong>错误: </strong>rpc 结果错误", 5000);
          }
          ...
          if (finished_tasks_list === undefined) {
            ...
            $.each(result, function(i, e) {
              ...
              if (ARIA2.finish_notification) {
                YAAW.notification("Aria2 任务完成", e.title);
              }
              finished_tasks_list.push(e.gid);
            });
          }
    ...
    pause_all: function() {
      ARIA2.request("pauseAll", [],
        function(result) {
          ...
          ARIA2.refresh();
          main_alert("alert-info", "所有任务已暂停 请等待", 2000);
        }
      );
    },
    ...
    unpause_all: function() {
      ARIA2.request("unpauseAll", [],
        function(result) {
          ...
          ARIA2.refresh();
          main_alert("alert-info", "所有任务已恢复", 2000);
        }
      );
    },
    ...
    purge_download_result: function() {
      ARIA2.request("purgeDownloadResult", [],
        function(result) {
          ...
          ARIA2.refresh();
          main_alert("alert-info", "已删除所有 完成/错误 的任务", 2000);
        }
      );
    },
      ...
      ARIA2.request("changeOption", [gid, options],
        function(result) {
          if (!result.result) {
            ...
            main_alert("alert-success", "选项已修改", 2000);
          }
        }
      );
    },
    ...
    auto_refresh: function(interval) {
      ...
      interval_id = window.setInterval(function() {
        ...
        if (select_lock) {
          if (need_refresh) {
            main_alert("", "任务列表已变化 点击【刷新】来更新列表");
          }
        } else {
          if (need_refresh) {
            ARIA2.refresh();
